  ...
  return (
    <div className="font-inter text-white bg-[#0A0D1A]">
      ...
      <header className="fixed w-full z-50 bg-[#0A0D1A]/80 backdrop-blur-md border-b border-white/10">
        ...
        {menuOpen && (
          <motion.nav
            ...
            exit={{ opacity: 0, y: -10 }}
            className="md:hidden bg-[#0A0D1A]/95 backdrop-blur-md border-t border-white/10 text-center py-6 space-y-4"
          >
            <Link
              to="/o-nas"
              onClick={() => setMenuOpen(false)}
              className="block hover:text-[#00A8FF] transition"
            >
              O nas
            </Link>
            <Link
            ...
            <Link
              to="/wycena"
              onClick={() => setMenuOpen(false)}
              className="inline-block px-6 py-2 rounded-lg bg-[#00A8FF] text-white font-semibold shadow-md hover:scale-105 transition"
            >
              Zamów wycenę
            </Link>
          </motion.nav>
      ...
      <section className="relative flex flex-col justify-center items-center min-h-screen text-center px-6 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-[#001a66] via-[#0033FF] to-[#000a33] bg-[length:200%_200%] animate-gradient"></div>
      ...
      {/* ---------------- WHY US ---------------- */}
      <section className="py-20 bg-[#0A0D1A] text-center px-6">
          ...
          {[
            {
              ...
              desc: "Korzystamy wyłącznie z certyfikowanych materiałów i sprawdzonych ekip.",
            },
            {
              icon: <Clock size={36} />,
              title: "Terminowość",
              desc: "Każdy projekt realizujemy zgodnie z harmonogramem i założonym budżetem.",
            },
            {
              icon: <FileText size={36} />,
              title: "Transparentność",
              desc: "Pełna przejrzystość kosztów i etapów inwestycji.",
            },
            {
              icon: <ThumbsUp size={36} />,
              title: "Zaufanie klientów",
              desc: "Wysokie oceny i liczne rekomendacje naszych inwestorów.",
            },
          ].map((item, i) => (
            <motion.div
              key={i}
              whileHover={{ y: -6 }}
              className="p-8 bg-white/5 border border-white/10 rounded-2xl shadow-lg hover:bg-white/10 transition"
            >
